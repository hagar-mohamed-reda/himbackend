
    <div class="w3-modal" style="display: block" id="printModal" >
        <div class="w3-modal-content w3-white w3-round w3-card w3-padding" style="direction: rlt" >
            <div class="w3-center w3-large w3-text-red"  style="direction: rtl" >
               <b> يتم طباعة  خدمة <?php echo e($name); ?>   </b>
            </div>
            <br>
            <div class="w3-center" >
                <button class="w3-button w3-indigo" onclick="printPage()" >طباعة</button>
            </div>
        </div>
    </div>
<?php /**PATH /home/pnpskkmy/public_html/him/resources/views/services/print.blade.php ENDPATH**/ ?>
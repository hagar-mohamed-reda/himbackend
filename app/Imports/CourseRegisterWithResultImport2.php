...
class CourseRegisterWithResultImport2 implements ToModel {
    ...
    public function result(array $row) {
        ...
        $mid_degree = str_replace(" ", "", $row[1] ?? '');

        try {
            $course = Course::where('id', $this->courseId ?? '')->first();
            $student = Student::where('code', $row[0] ?? '')->first();
            ...
            if(!$student || !$course )
             ...
             $resource = StudentResult::query()
                 ...
                 if ($resource) {
                     if($mid_degree != null && is_numeric($mid_degree)){
                ...
                //              $resource->calculateCourseGpa();
                            ...
                            //  $academic_student->startCalculateGpa();

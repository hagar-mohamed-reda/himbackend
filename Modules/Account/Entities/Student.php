...
class Student extends StudentOrigin
{
    ...
    public function getAvailableCardsAttribute() {
     ...
     public function getAcademicYearExpenseDetailAttribute() {
         ...
         $academicYearExpensesDetails = $academicYearExpenses->details()->get();
 
        return $academicYearExpensesDetails;
    }
    
    public function getCurrentModelIdAttribute() {
         $student = Student::FindOrFail($this->id);
         $currentModelId = $student->getStudentBalance()->getCurrrentAcademicYearExpenseDetail();
         if($currentModelId != null)
         $currentModelId = $currentModelId->first()->id;
    ...
    public function getLastDiscountRequestAttribute() {
         DB::table('account_discount_requests')->where('student_id', $this->id)->where('paid', '!=', '1')->latest()->first();
    }
    
    public function getLastDiscountAttribute() {
        return DB::table('account_discounts')->where('student_id', $this->id)->where('completely_paid', '!=', '1')->sum('value');
    }
    ...
    public function getPaidValueAttribute() {
        // return $this->getStudentBalance()->getPaidValue() - $this->getDiscountTotalAttribute();
    ...
    public function toAcademicStudent() {
        return StudentAcademic::find($this->id);
    }
